...
class BayesianModel:
    ...
    def __init__(
        self,
        parallax_data: Union[pd.DataFrame, bool] = None,
        m_data: Union[dict, bool] = None,
        Li_data: Union[pd.DataFrame, bool] = None,
        nStars: int = None,
    ):
        ...
        if parallax_data is not None and not isinstance(parallax_data, bool):
            ...
            raise AttributeError(
                "Number of stars cannot be determined, please check input data"
            )
        ...
        with pm.Model() as model:
            ...
            if "age" in priors.keys():
                if priors["age"]["dist"] == "normal":
                    age = pm.Normal(
                        "age", mu=priors["age"]["mu"], sigma=priors["age"]["sigma"]
                    )
                elif priors["age"]["dist"] == "uniform":
                    age = pm.Uniform(
                        "age",
                        upper=priors["age"]["upper"],
                        lower=priors["age"]["lower"],
                    )
                else:
            ...
            if "distance" in priors.keys():
                if priors["distance"]["dist"] == "normal":
                    distance = pm.Normal(
                        "distance",
                        mu=priors["distance"]["mu"],
                        sigma=priors["distance"]["sigma"],
                        shape=self.nStars,
                    )
                elif priors["distance"]["dist"] == "uniform":
                    distance = pm.Uniform(
                        "distance",
                        upper=priors["distance"]["upper"],
                        lower=priors["distance"]["lower"],
                        shape=self.nStars,
                    )
                else:
            ...
            if self.parallax_data is not None:
                parallax_true = pm.Deterministic(
                    "parallax*", distance2parallax(distance / 1000)
                )  # Units factor: pc -> kpc
            ...
            # Likelihoods #
            ###############
            if self.parallax_data is not None and not isinstance(
                self.parallax_data, bool
            ):
                parallax_data = pm.ConstantData(
                    "parallax_data", self.parallax_data["data"]
                )
                parallax_obs = pm.Normal(
                    "parallax",
                    mu=parallax_true,
                    sigma=self.parallax_data["sigma"],
                    observed=parallax_data,
                )

            # Photometry
            if self.m_data is not None and not isinstance(self.m_data, bool):
                # Compute "observed" flux
                flux_data = pm.ConstantData(
                    "flux_data", m2flux(self.m_data["data"]).fillna(1)
                )
    ...
    def summary(self):
        ...
        try:
            ...
            raise AttributeError(
                "Bayesian Model cannot be found, please compile the model."
            ) from error
        ...
        try:
            with self._model:
                ...
                self.trace = pm.sample(
                    draws=draws, return_inferencedata=True, step=s, **kwards
                )
        except AttributeError as error:
            raise AttributeError(
                "Bayesian Model cannot be found, please compile the model."
    ...
    def sample_prior_predictive(
        self, samples: int = 500, return_inferencedata: bool = False
    ):
        ...
        try:
            ...
            raise AttributeError(
                "Bayesian Model cannot be found, please compile the model first."
            ) from error
        else:
    ...
    def generate_data(self, mode: str = "dist") -> dict:
        ...
        if self.parallax_data is not None:
            ...
            idxp = ~np.isnan(p)
            parallax_data = pd.DataFrame({"data": p, "idx": idxp})
        ...
        if self.Li_data is not None:
            ...
            idxLi = ~np.isnan(Li)
            Li_data = pd.DataFrame({"data": Li, "idx": idxLi})
        ...
        if kind == "age":
            ...
            try:
                ...
                for s in range(self.idata.prior["M*"].shape[1]):
                    M_prior = pd.DataFrame(
                        self.idata.prior["M*"][0, s].to_numpy(), columns=self.filters
                    )
            ...
            for s in range(self.idata.posterior["M*"].shape[1]):
                M_post = pd.DataFrame(
                    self.idata.posterior["M*"][0, s].to_numpy(), columns=self.filters
                )
            ...
            try:
                ...
                warnings.warn(
                    "Inference data does not contain posterior predictive samples."
                )
    ...
    def plot_QQ(self, var_name: str, fig, ax):
        ...
        if var_name in ["parallax", "Li"]:
            ...
            idx = [1, 5, 50, 80, 95, 99]
            sns.scatterplot(
                x=np.array(per_ppc)[idx],
                y=np.array(per_prior)[idx],
                ax=ax,
                color="black",
            )
            for i in idx:
    ...
    def plot_CMDiagram(
        self, prior_predictive: bool = True, posterior_predictive: bool = False
    ):
        """DEPRECATED"""
        fig, axs = plt.subplots(
            2, 1, figsize=(10, 9), sharex=True, gridspec_kw={"height_ratios": [2.5, 1]}
        )
        axs[0].invert_yaxis()
        if self.priors["age"]["dist"] == "normal":
            fig.suptitle(
                r"$p(age|I)$ ~ $\mathcal{{N}}(\mu = {mu}, \sigma = {sigma})$ Myr".format(
                    mu=self.priors["age"]["mu"], sigma=self.priors["age"]["sigma"]
                )
            )
        elif self.priors["age"]["dist"] == "uniform":
            fig.suptitle(
                r"$p(age|I)$ ~ $\mathcal{{U}}(a = {a}, b = {b})$ Myr".format(
                    a=self.priors["age"]["lower"], b=self.priors["age"]["upper"]
                )
            )
        ...
        if prior_predictive:
            ...
            if self.Li_data is not None or self.Li_data:
                ...
                size = (Li_prior - Li_prior.min()) / (
                    Li_prior.max() - Li_prior.min()
                ) * 100 + 50
                axs[1].scatter(G_prior - Grp_prior, Li_prior, alpha=0.7, s=50)
            ...
            for i in range(G_prior_all.shape[0]):
                ...
                idx = np.argsort(x)
                y = G_prior_all[i, :]
                axs[0].plot(x[idx], y[idx], alpha=0.3, c="c")
            axs[0].legend()
